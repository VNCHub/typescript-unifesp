...
(() => {
  ...
  async function enrollRequest(alunoId, materiaId) {
  ...
  async function unenrollRequest(id) {
    return fetch(`${API_ENROLLMENT}/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${getToken()}` }
    });
  }
  async function getClassNoteById(id) {
    const res = await fetch(`${API_CLASSNOTES}/${id}`, {
      headers: { Authorization: `Bearer ${getToken()}` }
    });
    return res.json();
  }
  async function createClassNote(note) {
  ...
  async function updateClassNote(id, note) {
  ...
  var API_SUBJECTS, API_ENROLLMENT, API_ME, API_CLASSNOTES;
  var init_HomeApi = __esm({
    "src/frontend/HomeApi.ts"() {
      "use strict";
      API_SUBJECTS = "/api/subjects";
      API_ENROLLMENT = "/api/enrollment";
      API_ME = "/api/me";
      API_CLASSNOTES = "/api/classnotes";
    }
  });

  // src/frontend/HomeRender.ts
  ...
  function renderList(subjects, currentUser, enrollments) {
    ...
    for (const subject of subjects) {
      const enrollment = enrollments.find((e) => e.materiaId === subject.id);
  ...
  function getEnrollButton(subjectId, user, enrollment) {
    if (!user)
  ...
  var init_easymde_min = __esm({
    "node_modules/easymde/dist/easymde.min.css"() {
    }
  });

  // src/frontend/HomeFrontend.ts
  var import_easymde, HomeFrontend;
  var init_HomeFrontend = __esm({
    "src/frontend/HomeFrontend.ts"() {
      "use strict";
      init_HomeApi();
      init_HomeRender();
      import_easymde = __toESM(require_easymde());
      init_easymde_min();
      HomeFrontend = class {
        constructor() {
        ...
        async handleEnroll(subjectId) {
          ...
          if (!alunoId) return alert("Erro: usu\xE1rio sem aluno associado");
          ...
          this.userEnrollments = await fetchEnrollments(this.currentUser.id);
          await this.loadAndRender();
          alert("Matr\xEDcula realizada!");
        }
        async handleUnenroll(id) {
          ...
          this.userEnrollments = await fetchEnrollments(this.currentUser.id);
          await this.loadAndRender();
          alert("Desmatr\xEDcula realizada!");
        }
        async openDetailsModal(subjectId) {
      ...
      `;
        ...
        initializeEditor(initialContent = "") {
          ...
          this.noteEditor = new import_easymde.default({
            element: textarea,
            spellChecker: false,
            autofocus: true,
            placeholder: "Digite o conte\xFAdo da aula em Markdown...",
            minHeight: "300px",
            status: false,
            toolbar: [
              "bold",
        ...
        async handleNewNote(enrollmentId) {
          ...
          document.querySelector("#save-classnote-btn").onclick = () => this.saveNewNote();
        }
        async saveNewNote() {
          const titulo = document.querySelector("#note-title-input").value.trim();
          const markdown = this.noteEditor?.value() || "";
          if (!titulo) return alert("Insira um t\xEDtulo.");
        ...
        async handleEditNote(id) {
          ...
          document.querySelector("#note-title-input").value = note.titulo;
          if (this.noteEditor) {
            this.noteEditor.toTextArea();
            this.noteEditor = null;
          }
          ...
          document.querySelector("#save-classnote-btn").onclick = () => this.saveEditedNote(id);
        }
        ...
        async handleViewNote(id) {
          const note = await getClassNoteById(id);
          console.log(note);
          if (!note) return alert("Erro ao carregar nota.");
          document.querySelector("#view-note-title").textContent = note.titulo;
          document.querySelector("#view-note-date").textContent = new Date(note.data).toLocaleDateString("pt-BR", {
            day: "2-digit",
            month: "long",
            year: "numeric"
          });
        ...
        async loadClassNotes(enrollmentId) {
